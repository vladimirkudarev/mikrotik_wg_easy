...
<body>
  ...
  <script>
    ...
    async function editClient(id){const c=await api(clientPath(id));currentEditId=id;editTitle.textContent="Редактировать: "+c.name;const fields=[["interface","Interface"],["name","Имя"],["address","Client address"],["dns","DNS"],["endpoint","Endpoint"],["allowed_ips","Allowed IPs"],["keepalive","Keepalive"],["comment","Comment"]];editFields.innerHTML=fields.map(([k,label])=>'<div class="field"><label for="e_'+k+'"><span>'+esc(label)+'</span></label><input id="e_'+k+'" value="'+esc(c[k]||'')+'"></div>').join("");editDlg.showModal()}
